...
final disabledModulesProvider = Provider<Set<AppModule>>((ref) {
  if (ref.watch(authStateProvider).valueOrNull == null) return const {};
  return ref.watch(citySettingsProvider).valueOrNull?.disabledModules ??
      const {};
});

final moduleEnabledProvider = Provider.family<bool, AppModule>((ref, module) {
  return !ref.watch(disabledModulesProvider).contains(module);
});
